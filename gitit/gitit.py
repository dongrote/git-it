...
def pre_push_pull_check():
    if git.has_unstaged_changes():
        print 'current working tree has unstaged changes.'
        return False
    if git.has_uncommitted_changes():
        print 'current working tree has uncommitted changes.'
        return False
    return True
...
class Gitit:
    ...
    def push(self):
        auto_stash = False
        if not pre_push_pull_check():
            auto_stash = True
            if git.stash_push():
                print >>sys.stderr, 'Error auto-stashing. Aborting.'
                sys.exit(1)
        ...
        if auto_stash:
            if git.stash_pop():
                print >>sys.stderr, 'There was an error auto-stash popping.'
